...
import static ua.com.merchik.merchik.Options.Controls.OptionControlTaskAnswer.countChar;
                ...
                if (theme.getTp().equals("2")) {
                    ...
                    || (!item.author.equals(item.lastAnswerUserId) && countChar(item.lastAnswer, ' ') < 2)) {
                        ...
                        if (answer.length() < 10) {
                            ...
                            String msg = context.getString(R.string.option_control_135330_two_space_tar_comment);
                            massageToUser = msg;
                            spannableStringBuilder.append(msg).append(": ").append(createLinkedString(item.id1c, item.id)).append("\n");
                            result.add(item);
                        }
...
//            }
            if (tarList.size() == 0) {
                ...
                spannableStringBuilder.append("\n").append("Есть замечание по ответам на ").append(String.valueOf(result.size())).append(" активных рекламаций. Вы должны сперва исправить " +
