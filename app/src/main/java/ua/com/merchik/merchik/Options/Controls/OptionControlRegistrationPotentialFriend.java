...
public class OptionControlRegistrationPotentialFriend<T> extends OptionControl {
    public int OPTION_CONTROL_REGISTRATION_POTENTIAL_CLIENT_ID = 136101;

    public boolean signal = false;
    private boolean report20 = false;
    private int err = 0;
    private String dateDiapason = "";
//    private int count = 0;
    private StringBuilder potentialFriendMsg = new StringBuilder();
    ...
    public OptionControlRegistrationPotentialFriend(Context context, T document, OptionsDB optionDB, OptionMassageType msgType, Options.NNKMode nnkMode, UnlockCodeResultListener unlockCodeResultListener) {
        try {
    ...
    private void executeOption() {
        List<SMSPlanSDB> smsPlanSDBS =  SQL_DB.smsPlanDao().getAll(null, dtFrom, dtTo, 933, null, null, "Привет", wpDataDB.getUser_id());
        List<SMSLogSDB> smsLogSDBS = SQL_DB.smsLogDao().getAll(null, dtFrom, dtTo, 933, null, null, "Привет", wpDataDB.getUser_id());

        if ((smsPlanSDBS == null || smsPlanSDBS.isEmpty()) && (smsLogSDBS == null || smsLogSDBS.isEmpty())) {
            potentialFriendMsg.append(dateDiapason).append(", сотрудник ").append(wpDataDB.getUser_txt()).append(" СМС-'пригласить друга' не отправлял.");
            signal = true;
        }

        UsersSDB user = SQL_DB.usersDao().getUserById(wpDataDB.getUser_id());
        if (err > 0 && user != null && (user.reportDate20 == null || wpDataDB.getDt().getTime() < user.reportDate20.getTime())) {
            potentialFriendMsg.append(", но он еще не провел своего 20-го отчета поэтому замечаний нет");
            signal = false;
        }

        stringBuilderMsg.append(potentialFriendMsg);
