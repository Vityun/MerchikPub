...
public interface SamplePhotoDao {
    ...
    @Query("SELECT * FROM sample_photo WHERE active = :active AND photoTp = :photoTp AND grpId = :grpId ORDER BY dtUpdate DESC, id ASC")
    List<SamplePhotoSDB> getPhotoLogActiveAndTpExactGroup(Integer active, Integer photoTp, Integer grpId);
